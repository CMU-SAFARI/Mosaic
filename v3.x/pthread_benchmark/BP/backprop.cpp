...
float* alloc_1d_dbl(int n) {
  ...
  new_al = (float*)malloc((unsigned)(n * sizeof(float)));
  if (new_al == NULL) {
...

{
  ...
  new_all = (float**)malloc((unsigned)(m * sizeof(float*)));
  if (new_all == NULL) {
...
// extern "C"
void bpnn_output_error(float* delta,
                       float* target,
                       float* output,
                       int nj,
                       float* err) {
...
BPNN* bpnn_read(char* filename) {
  ...
  fread((char*)&n1, sizeof(char), sizeof(char), rfile);
  fread((char*)&n2, sizeof(char), sizeof(char), rfile);
  fread((char*)&n3, sizeof(char), sizeof(char), rfile);
  new_f = bpnn_internal_create(n1, n2, n3);
  ...
  fread(mem, (unsigned)(sizeof(float)),
        (unsigned)((n1 + 1) * (n2 + 1) * sizeof(float)), rfile);

  for (i = 0; i <= n1; i++) {
    for (j = 0; j <= n2; j++) {
      fastcopy(&(new_f->input_weights[i][j]), &mem[memcnt], sizeof(float));
      memcnt += sizeof(float);
    }
  }
  free(mem);
  ...
  fread(mem, sizeof(float), (unsigned)((n2 + 1) * (n3 + 1) * sizeof(float)),
        rfile);
  for (i = 0; i <= n2; i++) {
    for (j = 0; j <= n3; j++) {
      fastcopy(&(new_f->hidden_weights[i][j]), &mem[memcnt], sizeof(float));
      memcnt += sizeof(float);
    }
  }
  free(mem);
  ...
  bpnn_zero_weights(new_f->input_prev_weights, n1, n2);
  bpnn_zero_weights(new_f->hidden_prev_weights, n2, n3);
