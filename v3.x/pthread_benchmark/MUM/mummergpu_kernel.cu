...
__global__ void mummergpuKernel(MatchCoord* match_coords,
                                char* queries,
  ...
  MatchCoord* result =
      match_coords + qryAddr - __umul24(qryid, min_match_len + 1);
  queries += qryAddr;
  ...
  RECORD_RESULT:
    ...
    set_result(cur, result, refpos - node.start, qry_match_len, min_match_len,
               FORWARD);
  ...
  NEXT_SUBSTRING:
    ...
    XPRINTF(
        " following suffix link. mustmatch:%d qry_match_len:%d sl:(%d,%d)\n",
...
__global__ void mummergpuRCKernel(MatchCoord* match_coords,
                                  char* queries,
                                  const int* queryAddrs,
                                  const int* queryLengths,
                                  const int numQueries,
  ...
  for (int qrystart = qlen; qrystart >= min_match_len; qrystart--, result++) {
    ...
    while ((c != '\0')) {
      ...
      if (cur.data == 0) {
        XPRINTF(" no edge\n");
        set_result(prev, result, 0, qry_match_len, min_match_len, REVERSE);
