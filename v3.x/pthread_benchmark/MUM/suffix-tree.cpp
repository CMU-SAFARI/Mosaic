...
class SuffixTree {
 public:
  ...
  void printTreeSorted(ostream& out,
                       SuffixNode* node,
...
void buildNodeTexture(SuffixNode* node,
  ...
  for (int i = 0; i < basecount; i++) {
    if (node->m_children[i]) {
      ...
      buildNodeTexture(node->m_children[i], nodeTexture, childrenTexture,
                       aux_data, refstr);
    }
  }
}
...
void buildSuffixTreeTexture(PixelOfNode** nodeTexture,
  ...
  *nodeTexture =
      (PixelOfNode*)calloc((*width) * (*height), sizeof(PixelOfNode));
  *childrenTexture =
      (PixelOfChildren*)calloc((*width) * (*height), sizeof(PixelOfChildren));

  *aux_data = (AuxiliaryNodeData*)calloc(SuffixNode::s_nodecount + 1,
                                         sizeof(AuxiliaryNodeData));
...
void printTreeTexture(const char* texfilename,
  ...
  texfile << "id\tx\ty\tstart\tend\ta.x\ta.y\tc.x\tc.y\tg.x\tg.y\tt.x\tt.y\t$."
             "x\t$.y"
          << endl;
  for (int i = 0; i < nodecount; i++) {
    TextureAddress myaddress(id2addr(i));

    texfile << i << "\t" << myaddress.x << "\t" << myaddress.y << "\t"
            << nodeTexture[i].start << "\t" << nodeTexture[i].end << "\t";
...
extern "C" void createTreeTexture(const char* refstr,
  ...
  // indexTree();
  buildSuffixTreeTexture(nodeTexture, childrenTexture, width, height, aux_data,
                         gtree->m_string);
...
extern "C" void getQueriesTexture(int qfile,
  ...
  *queryLengths = lengths;
  ...
  cerr << offsetspos << " queries (" << qrylen << " bp), need "
       << curr_mem_usage << " bytes on the GPU " << timer.str(true, 5) << endl;
}
...
void printNodeAlignments(const char* ref,
  ...
  if (isLeaf) {
    ...
    if (dollar.data) {
      printNodeAlignments(ref, page, queryflankingbase, dollar, qrypos, qrylen,
                          path, path_idx, rc);
    }
  }
}
