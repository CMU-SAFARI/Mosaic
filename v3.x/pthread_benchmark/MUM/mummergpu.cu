...
extern "C" void getReferenceString(const char* filename,
                                   char** refstr,
                                   size_t* reflen);
...
void buildReferenceTexture(Reference* ref,
  ...
  createTreeTexture(ref->str, &nodeTexture, &childrenTexture, &width, &height,
                    &aux_data, &num_nodes, NULL, NULL);
  ...
  ref->bytes_on_board =
      width * height * (sizeof(PixelOfNode) + sizeof(PixelOfChildren));
...
void loadReferenceTexture(MatchContext* ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_SAFE_CALL(cudaMallocArray((cudaArray**)(&ref->d_ref_tex_array),
                                   &refTextureDesc, ref->pitch, numrows));

    // ref->bytes_on_board += ref->pitch * numrows;

    CUDA_SAFE_CALL(cudaMemcpyToArray((cudaArray*)(ref->d_ref_tex_array), 0, 0,
                                     ref->h_ref_tex_array, numrows * ref->pitch,
    ...
    CUDA_SAFE_CALL(cudaBindTextureToArray(
        reftex, (cudaArray*)ref->d_ref_tex_array, refTextureDesc));
...
void loadReference(MatchContext* ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    // ref->bytes_on_board += ref->tex_width * ref->tex_height *
    // (sizeof(PixelOfNode));
    ...
    CUDA_SAFE_CALL(cudaBindTextureToArray(
        nodetex, (cudaArray*)ref->d_node_tex_array, nodeTextureDesc));
    ...
    CUDA_SAFE_CALL(cudaBindTextureToArray(
        childrentex, (cudaArray*)(ref->d_children_tex_array),
        childrenTextureDesc));
    fprintf(stderr, "done\n");
...
void loadQueries(MatchContext* ctx, cudaStream_t stream_app) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_SAFE_CALL(
        cudaMemcpyAsync((void*)queries->d_tex_array,
                        queries->h_tex_array + queries->h_addrs_tex_array[0],
                        queries->texlen, cudaMemcpyHostToDevice, stream_app));
    ...
    CUDA_SAFE_CALL(cudaMemcpyAsync(
        ...
        numQueries * sizeof(int), cudaMemcpyHostToDevice, stream_app));
...
void freeResults(MatchContext* ctx,
                 ReferencePage pages[],
...
void addPackedOutput(MatchContext* ctx,
                     packed_slot_array** curr_output,
                     packed_slot_array slot_array[]) {
  unsigned int numQueries = ctx->queries->count;
  if (*curr_output == NULL) {
    *curr_output = slot_array;
    ...
    for (int i = 0; i < numQueries; i++) {
      if (slot_array[i].num_slots) {
        ...
        (*curr_output)[i].slots = (packed_slot*)realloc(
            (*curr_output)[i].slots,
...
void printRCSlots(MatchContext* ctx,
                  ReferencePage pages[],
                  int qry,
                  packed_slot_array* slots) {
  ...
  for (int j = 0; j < slots->num_slots; ++j) {
    ...
    if (slot.coord.edge_match_length & FRMASK) {
      printAlignments(ctx->full_ref, &(pages[slot.page]),
                      h_tex_array + h_addrs_tex_array[qry], qrylen,
                      slot.coord.node, slot.qpos,
                      (slot.coord.edge_match_length & FRUMASK),
                      ctx->min_match_length, 1, ctx->forwardcoordinates);
    }
  }
}
int FOO;
void printForwardSlots(MatchContext* ctx,
                       ReferencePage pages[],
                       int qry,
                       packed_slot_array* slots) {
  char* h_tex_array = ctx->queries->h_tex_array;
  ...
  for (int j = 0; j < slots->num_slots; ++j) {
    ...
    if (!(slot.coord.edge_match_length & FRMASK)) {
      printAlignments(ctx->full_ref, &(pages[slot.page]),
                      h_tex_array + h_addrs_tex_array[qry], qrylen,
                      slot.coord.node, slot.qpos, slot.coord.edge_match_length,
                      ctx->min_match_length, 0, ctx->forwardcoordinates);
    }
  }
  FOO += slots->num_slots;
}

void printPackedResults(MatchContext* ctx,
                        ReferencePage pages[],
                        packed_slot_array slot_array[]) {
  unsigned int numQueries = ctx->queries->count;
  FOO = 0;
...
void packSlots(MatchContext* ctx,
               MatchResults* results,
               unsigned int page_num,
               packed_slot_array** slot_arrays,
               bool rc) {
  unsigned int numQueries = ctx->queries->count;
  ...
  *slot_arrays =
      (packed_slot_array*)calloc(numQueries, sizeof(packed_slot_array));
  ...
  for (int i = 0; i < numQueries; i++) {
    ...
    for (int p = 0; p < qlen; ++p) {
      ...
      int query_coord_begin =
          match_coord_addrs(i, h_addrs_tex_array[i], match_length);
      int query_coord_end =
          i < numQueries - 1
              ? match_coord_addrs(i + 1, h_addrs_tex_array[i + 1], match_length)
              : results->numCoords;

      int query_coord = query_coord_begin + p;

      if ((query_coord < query_coord_end) && (coords[query_coord].node > 1) &&
...
int getQueryBlock(MatchContext* ctx, size_t device_mem_avail) {
  ...
  getQueriesTexture(queries->qfile, &queryTex, &queryLen, &queryAddrs, &names,
                    &queryLengths, &numQueries, device_mem_avail,
                    ctx->min_match_length, ctx->reverse || ctx->forwardreverse);
...
int matchSubset(MatchContext* ctx,
  ...
  for (unsigned int i = 0; i < num_pages; ++i) {
    ...
    if (ctx->on_cpu) {
      ...
      if (alignRC) {
        mummergpuRCKernel<<<dimGrid, dimBlock, 0, stream_app>>>(
        ...
        mummergpuKernel<<<dimGrid, dimBlock, 0, stream_app>>>(
            ctx->results.d_match_coords, ctx->queries->d_tex_array,
            ctx->queries->d_addrs_tex_array, ctx->queries->d_lengths_array,
            numQueries, ctx->min_match_length);
      }
...
extern "C" int matchQueries(MatchContext* ctx,
  ...
  if (!ctx->on_cpu) {
    ...
    if (deviceCount != 1) {
      // fprintf(stderr, "Fatal error: no CUDA-capable device found,
      // exiting\n");
      // return -1;
    }
  ...
  for (int i = 0; i < num_reference_pages; ++i) {
    mem_avail =
        max((unsigned int)pages[i].ref.bytes_on_board, (unsigned int)mem_avail);
  }
