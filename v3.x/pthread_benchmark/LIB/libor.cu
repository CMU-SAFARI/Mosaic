...
int main_lib(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  cudaMemcpyToSymbolAsync(N, &h_N, sizeof(h_N), 0, cudaMemcpyHostToDevice,
                          stream_app);
  cudaMemcpyToSymbolAsync(Nmat, &h_Nmat, sizeof(h_Nmat), 0,
                          cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyToSymbolAsync(Nopt, &h_Nopt, sizeof(h_Nopt), 0,
                          cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyToSymbolAsync(delta, &h_delta, sizeof(h_delta), 0,
                          cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyToSymbolAsync(maturities, &h_maturities, sizeof(h_maturities), 0,
                          cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyToSymbolAsync(swaprates, &h_swaprates, sizeof(h_swaprates), 0,
                          cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyToSymbolAsync(lambda, &h_lambda, sizeof(h_lambda), 0,
                          cudaMemcpyHostToDevice, stream_app);
