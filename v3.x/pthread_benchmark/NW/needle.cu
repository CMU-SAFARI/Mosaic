...
void runTest(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  cudaMemcpyAsync(referrence_cuda, referrence, sizeof(int) * size,
                  cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyAsync(matrix_cuda, input_itemsets, sizeof(int) * size,
                  cudaMemcpyHostToDevice, stream_app);
  ...
  for (int i = 1; i <= block_width; i++) {
    dimGrid.x = i;
    dimGrid.y = 1;
    needle_cuda_shared_1<<<dimGrid, dimBlock, 0, stream_app>>>(
        referrence_cuda, matrix_cuda, matrix_cuda_out, max_cols, penalty, i,
        block_width);
  }
  ...
  for (int i = block_width - 1; i >= 1; i--) {
    dimGrid.x = i;
    dimGrid.y = 1;
    needle_cuda_shared_2<<<dimGrid, dimBlock, 0, stream_app>>>(
        referrence_cuda, matrix_cuda, matrix_cuda_out, max_cols, penalty, i,
        block_width);
  }
  ...
  cudaMemcpyAsync(output_itemsets, matrix_cuda, sizeof(int) * size,
