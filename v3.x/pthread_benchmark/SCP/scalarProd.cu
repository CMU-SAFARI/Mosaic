...
int main_SCP(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  cutilSafeCall(
      cudaMemcpyAsync(d_A, h_A, DATA_SZ, cudaMemcpyHostToDevice, stream_app));
  cutilSafeCall(
      cudaMemcpyAsync(d_B, h_B, DATA_SZ, cudaMemcpyHostToDevice, stream_app));
  ...
  scalarProdGPU<<<128, 256, 0, stream_app>>>(d_C, d_A, d_B, VECTOR_N,
                                             ELEMENT_N);
  ...
  cutilSafeCall(cudaMemcpyAsync(h_C_GPU, d_C, RESULT_SZ, cudaMemcpyDeviceToHost,
                                stream_app));
