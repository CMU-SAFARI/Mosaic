...
float pFL(Points *points, int *feasible, int numfeasible,
	  ...
	  int pid, pthread_barrier_t* barrier,cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
#ifdef ENABLE_THREADS
  pthread_barrier_wait(barrier);
#endif
  ...
  while (change/cost > 1.0*e) {
    ...
    for (i=0;i<iter;i++) {
	    ...
	    change += pgain(feasible[x], points, z, k, kmax, is_center, center_table, switch_membership,
												&serial, &cpu_gpu_memcpy, &memcpy_back, &gpu_malloc, &kernel,stream_app,mutexapp,flag);
	       ...
	       int pid, pthread_barrier_t* barrier,cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag )
{
  ...
  while(1) {
    ...
    cost = pFL(points, feasible, numfeasible,
	       z, &k, kmax, cost, (long)(ITER*kmax*log((float)kmax)), 0.1, pid, barrier, stream_app, mutexapp, flag);
      ...
      if( pid== 0)
	{
	 ...
	 improvement */
      ...
      cost = pFL(points, feasible, numfeasible,
		 z, &k, kmax, cost, (long)(ITER*kmax*log((float)kmax)), 0.001, pid, barrier,stream_app, mutexapp, flag);
    }
...
void* localSearchSub(void* arg_) {
  ...
  pkmedian(arg->points,arg->kmin,arg->kmax,arg->kfinal,arg->pid,arg->barrier,arg->stream_app, arg->mutexapp, arg->flag);

  return NULL;
}

void localSearch( Points* points, long kmin, long kmax, long* kfinal,cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag ) {
#ifdef PROFILE
  double t1 = gettime();
#endif
...
void streamCluster( PStream* stream, 
		    ...
		    long chunksize, long centersize, char* outfile, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  streamCluster(stream, kmin, kmax, dim, chunksize, clustersize, outfilename, stream_app, mutexapp, flag );
