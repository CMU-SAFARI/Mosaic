...
void create_bmp(unsigned* data) {
  ...
  for (unsigned y = 0; y < height; y++) {
    for (unsigned x = 0; x < width; x++) {
      ...
      out_bmp(x, (height - y - 1))->Green =
          (0x0000FF00 & data[x + y * width]) >> 8;
      out_bmp(x, (height - y - 1))->Blue =
          (0x00FF0000 & data[x + y * width]) >> 16;
      out_bmp(x, (height - y - 1))->Alpha =
          (0xFF000000 & data[x + y * width]) >> 24;
    }
  }
