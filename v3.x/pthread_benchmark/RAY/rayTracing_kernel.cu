...
typedef struct { float4 m[4]; } matrice4x4;
...
typedef struct Sphere {
  ...
  float r;   // rayon
  float R, V, B, A;
  /*Sphere() : C(make_float3(0.0f,0.0f,0.0f)), r(0.5f),
  rvba(make_float4(1.0f,0.0f,0.0f,1.0f)) { }
  Sphere(const float3 _C, float _r, const float4 _rvba) : C(_C), r(_r),
  rvba(_rvba) { }
  Sphere(const float3 _C, float _r) : C(_C), r(_r),
  rvba(make_float4(1.0f,0.0f,0.0f,1.0f)) { }*/
} Sphere;
...
/*__host__ __device__ void createNode(Node * n, Node * fg, Node * fd, const
Sphere & s)
{
  n->fg = fg;
  n->fd = fd;
...
#ifdef DEVICE_EMU
  printf("%d: rgba = %f %f %f %f\n", threadIdx.x, rgba.x, rgba.y, rgba.z,
         rgba.w);
#endif
...
#ifdef DEVICE_EMU
  printf("%d: rgba = %x %x %x %x\n", threadIdx.x, uint(rgba.x * 255),
         uint(rgba.y * 255), uint(rgba.z * 255), uint(rgba.w * 255));
#endif
...
/*__global__ void d_render(uint * d_output, uint imageW, uint imageH, float pas,
float df, float tPixel)
{
...
make_float3(M.m[1])*(float(y)-float(imageH)*0.5f)*tPixel;
         ...
         s2 = st;
                ...
                float4 f =
make_float4(0,1,0,1)*(dot(getNormale(R.A+R.u*t,s.C),(-1.0f)*R.u));
...
*/
/*__global__ void rayCast (uint * d_output, uint * d_temp, uint imageW, uint
imageH, float pas, float df)
...
/*__global__  __device__ void rayCalc(float3 * A, float3 * u, float * prof, uint
imageW, uint imageH, float df, float tPixel)
{
        uint x = __umul24(blockIdx.x, blockDim.x) + threadIdx.x;
    uint y = __umul24(blockIdx.y, blockDim.y) + threadIdx.y;
        uint id = x + y * imageW;
...
/*__global__  __device__ void rayTrace(uint * Obj, float * prof, float3 * A,
float3 * u, uint imageW, uint imageH, float pas, float df, uint nObj)
{
        uint x = __umul24(blockIdx.x, blockDim.x) + threadIdx.x;
    uint y = __umul24(blockIdx.y, blockDim.y) + threadIdx.y;
        uint id = x + y * imageW;
...
/*
__global__  __device__ void color(uint * result, uint * Obj, float * prof,
float3 * A, float3 * u, uint imageW, uint imageH, float pas)
{
        uint x = __umul24(blockIdx.x, blockDim.x) + threadIdx.x;
    uint y = __umul24(blockIdx.y, blockDim.y) + threadIdx.y;
        uint id = x + y * imageW;
        ...
        if( x < imageW && y < imageH )
        {
                ...
                if( t > 0.0f  && t < 1000.0f ) {
                        ...
                        s.C.x += pas;
                        float4 f =
make_float4(s.R,s.V,s.B,s.A)*(dot(getNormale(R.A+R.u*t,s.C),(-1.0f)*R.u));
...
#ifdef DEBUG_RT_CUDA
__device__ bool notShadowRay(float4* d_debug_float4,
                             uint* d_debug_uint,
                             int i,
                             Node* node,
                             float3 A,
...
#ifdef DEBUG_RT_CUDA
__global__ __device__ void render(float4* d_debug_float4,
                                  uint* d_debug_uint,
                                  uint* result,
                                  Node* dnode,
                                  uint imageW,
                                  ...
                                  float df)
#else
__global__ __device__ void render(uint* result,
                                  Node* dnode,
                                  uint imageW,
                                  uint imageH,
                                  float pas,
...
//          pile[i].x, pile[i].y, pile[i].z, pile[i].w);
//          printf("%d: i=%d, color = %e %e %e %e\n", threadIdx.x, i, color.x,
...
//          printf("%d: i=%d, Np = %e %e %e\n", threadIdx.x, i, Np.x, Np.y,
//          Np.z);
//          printf("%d: i=%d, dot(Np,L) = %e\n", threadIdx.x, i, dot(Np,L));
// printf("%d: i=%d, notShadowRay(cnode,P,L,pas) = %d\n", threadIdx.x, i, (int)
// notShadowRay(cnode,P,L,pas));

#endif
#ifdef DEBUG_RT_CUDA
// d_debug_float4[threadIdx.x*16+i*3+0]= pile[i];
//          d_debug_float4[threadIdx.x*16+i*8+0]= color;
//          d_debug_float4[threadIdx.x*16+i*8+1].x=
//          P.x;d_debug_float4[threadIdx.x*16+i*8+1].y=
//          P.y;d_debug_float4[threadIdx.x*16+i*8+1].z= P.z;
//          d_debug_float4[threadIdx.x*16+i*8+2].x=
//          L.x;d_debug_float4[threadIdx.x*16+i*8+2].y=
//          L.y;d_debug_float4[threadIdx.x*16+i*8+2].z= L.z;
//          d_debug_float4[threadIdx.x*16+i*8+3].x=
//          V.x;d_debug_float4[threadIdx.x*16+i*8+3].y=
//          V.y;d_debug_float4[threadIdx.x*16+i*8+3].z= V.z;
//          d_debug_float4[threadIdx.x*16+i*8+4].x=
//          N.x;d_debug_float4[threadIdx.x*16+i*8+4].y=
//          N.y;d_debug_float4[threadIdx.x*16+i*8+4].z= N.z;
//          d_debug_float4[threadIdx.x*16+i*8+5].x=
//          Np.x;d_debug_float4[threadIdx.x*16+i*8+5].y=
//          Np.y;d_debug_float4[threadIdx.x*16+i*8+5].z= Np.z;
//          d_debug_float4[threadIdx.x*16+i*8+6].x= dot(Np,L);
// d_debug_float4[threadIdx.x*16+i*8+7].x= (float) notShadowRay(cnode,P,L,pas);
#endif
#ifdef DEBUG_RT_CUDA
        if (dot(Np, L) > 0.0f &&
            notShadowRay(d_debug_float4, d_debug_uint, i, cnode, P, L, pas)) {
#else
...
//    pile[0].z, pile[0].w);
//    printf("%d: pile[1] = %e %e %e %e\n", threadIdx.x, pile[1].x, pile[1].y,
//    pile[1].z, pile[1].w);
//    printf("%d: pile[2] = %e %e %e %e\n", threadIdx.x, pile[2].x, pile[2].y,
//    pile[2].z, pile[2].w);
//    printf("%d: pile[3] = %e %e %e %e\n", threadIdx.x, pile[3].x, pile[3].y,
//    pile[3].z, pile[3].w);
//    printf("%d: pile[4] = %e %e %e %e\n", threadIdx.x, pile[4].x, pile[4].y,
//    pile[4].z, pile[4].w);
#endif
    for (int i(n - 1); i > 0; i--)
...
/*__global__  __device__ void renderPixel(uint * result, Node * dnode, uint
imageW, uint imageH, float pas, float df)
{
...
make_float3(M.m[1])*(float(y)-float(imageH)*0.5f)*tPixel;
                ...
                if( tid == Obj && t > 0.0f ) {
                        s = node.s;
                        s.C.x += pas;
                        float3 P(R.A+R.u*t), L(normalize(make_float3(0,1,2)-P)),
V(-1*R.u);
                        float3 N(node.fg?getNormaleP(P):getNormale(P,s.C));
                        if( dot(N,L) > 0.0f ) {
                                Color =
0.5f*make_float4(s.R,s.V,s.B,s.A)*(max(0.0f,dot(N,L)));
            #ifdef FIXED_CONST_PARSE
                                Color +=
0.8f*make_float4(1.0f,1.0f,1.0f,1.0f)*pow(max(0.0f,min(1.0f,dot(2.0f*N*dot(N,L)-L,V))),20.0f);
            #else
            Color +=
0.8f*make_float4(1.0f,1.0f,1.0f,1.0f)*float2int_pow20(max(0.0f,min(1.0f,dot(2.0f*N*dot(N,L)-L,V))));
            #endif
                        }
