...
int compute_tran_temp(float *MatrixPower,float *MatrixTemp[2], int col, int row, \
		int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag) 
{
	...
	for (t = 0; t < total_iterations; t+=num_iterations) {
            ...
            src = dst;
            dst = temp;
            calculate_temp<<<dimGrid, dimBlock,0, stream_app>>>(MIN_hotspot(num_iterations, total_iterations-t), MatrixPower,MatrixTemp[src],MatrixTemp[dst],\
...
void run(cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
    ...
    int ret = compute_tran_temp(MatrixPower,MatrixTemp,grid_cols,grid_rows, \
	 total_iterations,pyramid_height, blockCols, blockRows, borderCols, borderRows, stream_app, mutexapp, flag);
