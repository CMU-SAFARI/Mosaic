...
void copy(T* dst, T* src, int N, cudaStream_t stream_app) {
  CUDA_SAFE_CALL(cudaMemcpyAsync((void*)dst, (void*)src, N * sizeof(T),
                                 cudaMemcpyDeviceToDevice, stream_app));
}
...
__device__ inline void compute_velocity(float& density,
                                        float3& momentum,
                                        float3& velocity) {
...
__device__ inline float compute_pressure(float& density,
                                         float& density_energy,
                                         float& speed_sqd) {
...
__global__ void cuda_compute_step_factor(int nelr,
                                         float* variables,
                                         float* areas,
                                         float* step_factors) {
  const int i = (blockDim.x * blockIdx.x + threadIdx.x);
...
void compute_step_factor(int nelr,
  ...
  dim3 Dg(nelr / block_length), Db(block_length);
  cuda_compute_step_factor<<<Dg, Db, 0, stream_app>>>(nelr, variables, areas,
                                                      step_factors);
 ...
 *
*/
__global__ void cuda_compute_flux(int nelr,
...
void time_step(int j,
               ...
               cudaStream_t stream_app) {
  dim3 Dg(nelr / block_length), Db(block_length);
  cuda_time_step<<<Dg, Db, 0, stream_app>>>(j, nelr, old_variables, variables,
                                            step_factors, fluxes);
...
int main_cfd(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  // set far field conditions and load them into constant memory on the gpu
  {
    ...
    CUDA_SAFE_CALL(cudaMemcpyToSymbolAsync(ff_variable, h_ff_variable,
                                           NVAR * sizeof(float), 0,
                                           cudaMemcpyHostToDevice, stream_app));
    CUDA_SAFE_CALL(cudaMemcpyToSymbolAsync(
        ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_x,
        sizeof(float3), 0, cudaMemcpyHostToDevice, stream_app));
    CUDA_SAFE_CALL(cudaMemcpyToSymbolAsync(
  ...
  std::cout << (cutGetAverageTimerValue(timer) / 1000.0) / iterations
