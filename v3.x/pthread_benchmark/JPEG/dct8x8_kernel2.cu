...
__constant__ float C_a = 1.387039845322148f;  //!< a = (2^0.5) * cos(    pi /
                                              //! 16);  Used in forward and
//! inverse DCT.
__constant__ float C_b = 1.306562964876377f;  //!< b = (2^0.5) * cos(    pi /
                                              //! 8);  Used in forward and
//! inverse DCT.
__constant__ float C_c = 1.175875602419359f;  //!< c = (2^0.5) * cos(3 * pi /
                                              //! 16);  Used in forward and
//! inverse DCT.
__constant__ float C_d = 0.785694958387102f;  //!< d = (2^0.5) * cos(5 * pi /
                                              //! 16);  Used in forward and
//! inverse DCT.
__constant__ float C_e = 0.541196100146197f;  //!< e = (2^0.5) * cos(3 * pi /
                                              //! 8);  Used in forward and
//! inverse DCT.
__constant__ float C_f = 0.275899379282943f;  //!< f = (2^0.5) * cos(7 * pi /
                                              //! 16);  Used in forward and
//! inverse DCT.
...
* \param Step			[IN] - Value to add to ptr to access other
*elements
*
* \return None
*/
...
__global__ void CUDAkernel2DCT(float* SrcDst, int ImgStride) {
  ...
  // SrcDst += FMUL(FMUL(blockIdx.y, KER2_BLOCK_HEIGHT) + OffsThreadInCol,
  // ImgStride) + FMUL(blockIdx.x, KER2_BLOCK_WIDTH) + OffsThreadInRow;
  reg1 = blockIdx.y;
  reg1 <<= KER2_BH_LOG2;
  ...
  // float *bl_ptr = block + FMUL(OffsThreadInCol, KER2_SMEMBLOCK_STRIDE) +
  // OffsThreadInRow;
  reg1 = OffsThreadInCol;
