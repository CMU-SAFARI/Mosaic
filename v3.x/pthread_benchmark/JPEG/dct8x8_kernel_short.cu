...
*shared memory).
*
* \param SrcDst			[IN/OUT] - Pointer to the first element of
*vector
* \param Stride			[IN] - Value to add to ptr to access other
*elements
*
* \return None
*/
...
*memory).
*
* \param V8			[IN/OUT] - Pointer to the first two elements of
*vector
*
* \return None
*/
...
__global__ void CUDAkernelShortDCT(short* SrcDst, int ImgStride) {
  ...
  // int OffsThrRowPermuted = bankID & 0xFFFFFFE0 | ((bankID << 1) | (bankID >>
  // 4) & 0x1) & 0x1F;
  ...
  // SrcDst += FMUL(FMUL(blockIdx.y, KERS_BLOCK_HEIGHT) + OffsThreadInCol,
  // ImgStride) + FMUL(blockIdx.x, KERS_BLOCK_WIDTH) + FMUL(OffsThreadInRow, 2);
  reg1 = blockIdx.y;
  reg1 <<= KERS_BH_LOG2;
  ...
  // short *bl_ptr = block + FMUL(block_y_offs, KERS_SMEMBLOCK_STRIDE) +
  // FMUL(OffsThreadInRow, 2);
  reg1 = block_y_offs;
