...
#define FRAME_GET(n, x, y) (frame_loc[((n) << 4) + ((y) << 2) + (x)])
#define FRAME_PUT_1(n, x, value) (frame_loc[((n) << 4) + (x)] = value)

/* Macros to access temporary SAD storage in shared memory */
#define SAD_LOC_GET(blocknum, pos) (sad_loc[(blocknum)*MAX_POS_PADDED + (pos)])
...
__global__ void mb_sad_calc(unsigned short* blk_sad,
  ...
  if ((mb_x < mb_width) && (mb_y < mb_height)) {
    ...
    for (search_pos = search_pos_base; search_pos < search_pos_end;
         search_pos += 3) {
  ...
  if ((threadIdx.x >> 5) < (THREADS_W * THREADS_H)) {
    ...
    if ((mb_x < mb_width) && (mb_y < mb_height)) {
      ...
      blk_sad +=
          (__umul24(__umul24(mb_width, mb_height), 25) +
           (__umul24(mb_y, mb_width) + mb_x) * 16 + (4 * block_y + block_x)) *
          MAX_POS_PADDED;
