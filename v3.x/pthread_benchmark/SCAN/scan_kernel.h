...
__global__ void reduce(const T* __restrict__ g_idata,
  ...
  int block_stop =
      (blockIdx.x == gridDim.x - 1) ? n : block_start + region_size;
...
__device__ T scanLocalMem(const T val, volatile T* s_data) {
  ...
  if (threadIdx.x < 32) {
    ...
    int idx = ((blockDim.x * 2) - 32) + threadIdx.x;
    s_data[idx] = sum;
  ...
  s_data[threadIdx.x + pad] +=
      s_data[((blockDim.x * 2) - 32) + (threadIdx.x / nsteps) - 1];
  __syncthreads();
...
__global__ void bottom_scan(const T* __restrict__ g_idata,
                            T* __restrict__ g_odata,
