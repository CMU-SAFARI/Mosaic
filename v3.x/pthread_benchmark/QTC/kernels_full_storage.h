...
inline __device__ int find_closest_point_to_cluster(int seed_point,
                                                    int latest_point,
                                                    char* Ai_mask,
  ...
  for (int i = 0; i + tid < max_degree; i += curThreadCount) {
    ...
    if (pnt2_indx >= 0 && 0 == Ai_mask[pnt2_indx] &&
        0 == clustered_pnts_mask[pnt2_indx]) {
      float dist_to_new_point = 0, diameter = 0;

      float curr_dist_to_clust = dist_to_clust[i + tid];
      if (can_use_texture) {
        dist_to_new_point = tex2D(texDistance, float(latest_point) + 0.5f,
                                  float(pnt2_indx) + 0.5f);
      } else {
      ...
      // The point that leads to the cluster with the smallest diameter is the
      // closest point
...
#define CHECK_POINT_FOR_PROXIMITY(_CAND_PNT_, _I_, _CAND_PNT_DIST_)            \
  do {                                                                         \
    /* Negative index means that there are no more points connected to the     \
     * seed */                                                                 \
    if ((_CAND_PNT_) >= 0) {                                                   \
      if (0 != Ai_mask[(_CAND_PNT_)] ||                                        \
...
inline __device__ int generate_candidate_cluster_full_storage(
    int seed_point,
    int degree,
    char* Ai_mask,
    float* work,
  ...
  // Put the seed point in the candidate cluster and mark it as taken in the
  // candidate cluster mask Ai_mask.
  flag = true;
  cnt = 1;
  if (0 == tid) {
  ...
  // If we are only computing the cardinality of each cluster, we don't need to
  ...
  if (NULL == candidate_cluster) {
    ...
    while ((cnt < point_count) && flag) {
      ...
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_3, (2 * curThreadCount + tid),
                                cp_dist_3);
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_4, (3 * curThreadCount + tid),
                                cp_dist_4);
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_5, (4 * curThreadCount + tid),
                                cp_dist_5);
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_6, (5 * curThreadCount + tid),
                                cp_dist_6);
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_7, (6 * curThreadCount + tid),
                                cp_dist_7);
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_8, (7 * curThreadCount + tid),
                                cp_dist_8);

      CHECK_POINT_FOR_PROXIMITY(cand_pnt_9, (8 * curThreadCount + tid),
                                cp_dist_9);
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_10, (9 * curThreadCount + tid),
                                cp_dist_10);
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_11, (10 * curThreadCount + tid),
                                cp_dist_11);
      CHECK_POINT_FOR_PROXIMITY(cand_pnt_12, (11 * curThreadCount + tid),
                                cp_dist_12);
  ...
  } else {  // if we are interested in the actual members of the cluster, put
            // them in the "candidate_cluster[]" array.

    while ((cnt < point_count) && flag) {
      int min_G_index;
      min_G_index = find_closest_point_to_cluster(
          seed_point, latest_point, Ai_mask, clustered_pnts_mask, work,
          indr_mtrx, dist_to_clust, point_count, N0, max_degree, threshold,
          can_use_texture);
      if (min_G_index >= 0) {
        if (0 == tid) {
          Ai_mask[min_G_index] = 1;
