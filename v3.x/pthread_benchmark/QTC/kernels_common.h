...
#include <math.h>
//#include <float.h> // float.h should be included by math.h and we need it for
// FLT_MAX
...
__global__ void QTC_device(float* dist_matrix,
                           char* Ai_mask,
                           char* clustered_pnts_mask,
                           int* indr_mtrx,
                           int* cluster_cardinalities,
                           int* ungrpd_pnts_indr,
                           float* dist_to_clust,
                           int* degrees,
                           int point_count,
                           int N0,
...
__device__ int generate_candidate_cluster_compact_storage(
    int seed_point,
    int degree,
    char* Ai_mask,
...
__device__ int generate_candidate_cluster_full_storage(
    int seed_point,
    int degree,
    char* Ai_mask,
    float* work,
...
__device__ int find_closest_point_to_cluster(int seed_point,
                                             int latest_point,
                                             char* Ai_mask,
                                             char* clustered_pnts_mask,
                                             float* work,
...
inline __device__ int closest_point_reduction(float min_dist,
                                              float threshold,
                                              int closest_point) {
  ...
  if (tid == 0) {
    for (int j = 1; j < curThreadCount; j++) {
      ...
      // look for a point that is closer, or equally far, but with a smaller
      // index.
      if ((dist < min_dist) ||
          (dist == min_dist && point_index_array[j] < point_index_array[0])) {
        min_dist = dist;
...
__global__ void reduce_card_device(int* cardinalities, int TB_count) {
...
__global__ void compute_degrees(int* indr_mtrx,
                                int* degrees,
                                int N0,
                                int max_degree) {
...
__global__ void update_clustered_pnts_mask(char* clustered_pnts_mask,
                                           char* Ai_mask,
                                           int N0) {
  int tid = threadIdx.x;
  int curThreadCount = blockDim.x * blockDim.y * blockDim.z;

  // If a point is part of the latest winner cluster, then it should be marked
  // as
...
__global__ void trim_ungrouped_pnts_indr_array(int seed_index,
                                               int* ungrpd_pnts_indr,
                                               float* dist_matrix,
                                               int* result_cluster,
                                               char* Ai_mask,
  ...
  if (matrix_type_mask & FULL_STORAGE_MATRIX) {
    (void)generate_candidate_cluster_full_storage(
        seed_index, degree, Ai_mask, dist_matrix, clustered_pnts_mask,
        indr_mtrx, dist_to_clust, point_count, N0, max_degree, result_cluster,
        threshold, can_use_texture);
  } else {
  ...
  for (int i = 0; i + tid < point_count; i += curThreadCount) {
    // Have all threads make a coalesced read of contiguous global memory and
    // copy the points assuming they are all good.
    tmp_pnts[tid] = ungrpd_pnts_indr[i + tid];
    int pnt = tmp_pnts[tid];
    // If a point is bad (which should not happen very often), raise a global
    // flag so that thread zero fixes the problem.
    if (1 == Ai_mask[pnt]) {
      flag_sh = true;
...
__global__ void QTC_device(float* dist_matrix,
  ...
  for (i = base_offset; i < point_count; i += total_thread_block_count) {
    ...
    int degree = degrees[seed_index];
    if (degree <= max_cardinality)
      continue;
    if (matrix_type_mask & FULL_STORAGE_MATRIX) {
      cnt = generate_candidate_cluster_full_storage(
          seed_index, degree, Ai_mask, dist_matrix, clustered_pnts_mask,
  ...
  // only one thread needs to write into the global memory since they all have
