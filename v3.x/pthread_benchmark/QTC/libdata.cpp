...
/*int read_BLAST_data(float **rslt_mtrx, int **indr_mtrx, int *max_degree, float
threshold, const char *fname, int maxN, int matrix_type_mask){
    FILE *ifp;
    float *dist_mtrx;
    int *index_mtrx;
    int prev_p1=-1, p1, p2;
...
float* generate_synthetic_data(float** rslt_mtrx,
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < N; j++) {
      ...
      float dist_sq =
          (p1_x - p2_x) * (p1_x - p2_x) + (p1_y - p2_y) * (p1_y - p2_y);
