...
#define COMPUTE_DIAMETER_WITH_POINT(_CAND_PNT_, _CURR_DIST_TO_CLUST_, _I_)     \
  ...
  do {                                                                         \
    ...
    /* if "_CAND_PNT_" is too far away, or already in Ai_mask, or in           \
     * clustered_points, ignore it. */                                         \
    if ((_CURR_DIST_TO_CLUST_ > threshold) || (0 != Ai_mask[(_CAND_PNT_)]) ||  \
    ...
    for (int j = last_index_checked; j < max_degree; j++) {                    \
      ...
      if (tmp_pnt == (_CAND_PNT_)) {                                           \
        if (can_use_texture) {                                                 \
          dist_to_new_point =                                                  \
              tex2D(texDistance, float(j) + 0.5f, float(latest_point) + 0.5f); \
        } else {                                                               \
...
inline __device__ int generate_candidate_cluster_compact_storage(
    int seed_point,
    int degree,
    char* Ai_mask,
  ...
  float curr_dist_to_clust_0, curr_dist_to_clust_1, curr_dist_to_clust_2,
      curr_dist_to_clust_3;
  float curr_dist_to_clust_4, curr_dist_to_clust_5, curr_dist_to_clust_6,
      curr_dist_to_clust_7;
  float curr_dist_to_clust_8, curr_dist_to_clust_9, curr_dist_to_clust_10,
      curr_dist_to_clust_11;
  int cand_pnt_i = -1;
  ...
  // Prefetch 12 points per thread, into registers, to reduce the memory
  // pressure (and delay) of
  // constantly going to memory to fetch these points inside the while() loop
  // that follows.
  do {
  ...
  // different threads might exit this loop at different times, so let them
  // catch up.
  __syncthreads();
  ...
  while ((cnt < point_count) && flag) {
    ...
    // The following loop implements the "find point pj s.t. diameter(Ai && pj)
    // is minimum"
