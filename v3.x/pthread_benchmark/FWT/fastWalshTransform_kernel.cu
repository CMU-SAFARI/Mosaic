...
////////////////////////////////////////////////////////////////////////////////
void fwtBatchGPU(float* d_Data, int M, int log2N, cudaStream_t stream_app) {
  const int THREAD_N = 256;

  int N = 1 << log2N;
  dim3 grid((1 << log2N) / (4 * THREAD_N), M, 1);
  for (; log2N > ELEMENTARY_LOG2SIZE; log2N -= 2, N >>= 2, M <<= 2) {
    fwtBatch2Kernel<<<grid, THREAD_N, 0, stream_app>>>(d_Data, d_Data, N / 4);
    cutilCheckMsg("fwtBatch2Kernel() execution failed\n");
  }

  fwtBatch1Kernel<<<M, N / 4, N * sizeof(float), stream_app>>>(d_Data, d_Data,
                                                               log2N);
...
void modulateGPU(float* d_A, float* d_B, int N, cudaStream_t stream_app) {
  modulateKernel<<<128, 256, 0, stream_app>>>(d_A, d_B, N);
}
