...
void regtileSgemm( char transa, char transb, int m, int n, int k, float alpha, const float *A, int lda, const float *B, int ldb, float beta, float *C, int ldc, cudaStream_t stream_app, pthread_mutex_t *mutexapp, bool flag)
{
  ...
  mysgemmNT<<<grid, threads, 0, stream_app>>>( A, lda, B, ldb, C, ldc, k, alpha, beta);
  CHECK_ERROR("mySgemm");
