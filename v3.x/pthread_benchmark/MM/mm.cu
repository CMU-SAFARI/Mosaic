...
int main_MM(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  // load A
  readColMajorMatrixFile((char*)"MM/matrix1.txt", matArow, matAcol, matA);
  // copy A to device memory
  A_sz = matArow * matAcol * sizeof(float);

  // load B^T
  readColMajorMatrixFile((char*)"MM/matrix2t.txt", matBcol, matBrow, matBT);
  ...
  /* Write C to file */
  pb_SwitchToTimer(&timers, pb_TimerID_IO);
  writeColMajorMatrixFile((char*)"MM/matrix3.txt", matArow, matBcol, matC);
  //}
